...
func _init_dialogue():
	...
	if _selected_dialogue:
		$Graph.scroll_offset = _selected_dialogue.scrolloffset
		...
		for node in _selected_dialogue.connections:
			$Graph.connect_node(node.from, node.from_port, node.to, node.to_port)
		nodes_colors_update()


func _to_graph_position(global_position : Vector2) -> Vector2:
	return (global_position + $Graph.scroll_offset) / $Graph.zoom

func _init_connections() -> void:
	assert($Graph.connect("node_selected", self, "_on_node_selected") == OK)
	assert($Graph.connect("_begin_node_move", self, "_on_node_begin_move") == OK)
	assert($Graph.connect("_end_node_move", self, "_on_node_end_move") == OK)
	assert($Popup.connect("mouse_entered", self, "_on_mouse_popup_entered") == OK)
	assert($Popup.connect("mouse_exited", self, "_on_mouse_popup_exited") == OK)
	assert($Graph.connect("gui_input", self, "_on_gui_input") == OK)
	assert($Popup.connect("id_pressed", self, "_on_popup_item_selected") == OK)
	assert($Graph.connect("connection_request", self, "_node_connection_request") == OK)
	assert($Graph.connect("disconnection_request", self, "_node_disconnection_request") == OK)
...
func _input(event: InputEvent) -> void:
	if event is InputEventMouseButton:
		if event.get_button_index() == BUTTON_LEFT and event.pressed:
			if $Popup.visible and not _mouse_over_popup:
				$Popup.hide()
...
func _show_popup() -> void:
	_build_popup()
	$Popup.set_position(_calc_popup_position())
...
func _build_popup_node_start() -> void:
	if get_node_or_null("Graph/NodeStart") == null:
		$Popup.add_item("Start", 1)
		
func _build_popup_node_sentence() -> void:
	$Popup.add_item("Sentence", 2)
...
func _build_popup_delete_nodes() -> void:
	$Popup.add_item("Clear All", 4)

func _calc_popup_position() -> Vector2:
	var pos_x = $Graph.rect_global_position.x + _mouse_position.x
	var pos_y = $Graph.rect_global_position.y + _mouse_position.y
...
func _calc_node_position() -> Vector2:
	var offset_x = ($Graph.scroll_offset.x + _mouse_position.x) / $Graph.get_zoom()
	var offset_y = ($Graph.scroll_offset.y + _mouse_position.y) / $Graph.get_zoom()
...
#	if _nodes_count() <= 0:
#		return
#	var newRes = dialogue.resource
#	newRes.nodes.clear()
#	newRes.connections.clear()
#	newRes.scrolloffset = $Graph.scroll_offset
#	for node in $Graph.get_children():
#		if node is GraphNode and node.has_method("save_data"):
#			dialogue.resource.nodes.append(node.save_data())
#	newRes.connections = $Graph.get_connection_list()
#	ResourceSaver.save(dialogue.resource.resource_path, newRes)
